...
public class ScaleCommand extends AnchoredTransformationCommand {
	...
	public void execute() {
		...
		while(iterateur.hasNext()){
			...
			double anchorPointX = this.getAnchorPoint(objects).getX();
			double anchorPointY = this.getAnchorPoint(objects).getY();
			
			t.scale(sx, sy);
			t.translate(anchorPointX * ( (1-sx) / sx ), anchorPointY * ( (1-sy) / sy ));
